import Footer from '../components/footer';
import styles from "../css/page.module.css";
import DownloadButton from './cv-download-button'

const classNames = require('classnames');

interface ContactMeProps {
  dictionary: { [key: string]: string }
}

export default function ContactMe({ dictionary } : ContactMeProps) {
  return (
    <div className={classNames(styles.main, styles.fontAiWritter)} id="contactMe">
      <div className={classNames(styles.textCenter, styles.spaceDown)}>
        <h3 className={classNames(styles.spaceDown)}>{dictionary.contactMePageTitle}</h3>
        <p className={classNames(styles.contactInfo)}>{dictionary.contactMePageDescription}</p>
      </div>
      <div className={classNames(styles.socialLogos)}>
        <a href="https://github.com/JosueELP" target="_blank" aria-label="Link to the github user profile of Josue Luna">
          <svg xmlns="http://www.w3.org/2000/svg" width="60" height="60" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-github"><path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4"/><path d="M9 18c-4.51 2-5-2-7-2"/></svg>
        </a>
        <a href="https://mx.linkedin.com/in/josuelunap" target="_blank" aria-label="Link to the linkedin user profile of Josue Luna">
          <svg xmlns="http://www.w3.org/2000/svg" width="60" height="60" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-linkedin"><path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z"/><rect width="4" height="12" x="2" y="9"/><circle cx="4" cy="4" r="2"/></svg>
        </a>
        <a href="mailto:josuee.lnap@gmail.com" target="_blank" aria-label="Link to send an email to Josue Luna">
          <svg xmlns="http://www.w3.org/2000/svg" width="60" height="60" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-mail"><rect width="20" height="16" x="2" y="4" rx="2"/><path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7"/></svg>
        </a>
      </div>
      <DownloadButton dictionary={dictionary}/>
      <Footer/>
    </div>
  );
}
